  ...
  try {
    switch (category) {
      ...
      case 'category':
        ...
        // 1. 獲取完整商品銷售數據（所有欄位，無限制）
        const categorySalesResponse = await fetch(`${baseUrl}/api/products/sales-data?limit=10000`, {
          headers: { 'Accept': 'application/json' }
        })
        
        // 2. 獲取完整商品主檔（獲取所有1610個商品）
        const categoryMasterResponse = await fetch(`${baseUrl}/api/products-master-sheets?limit=2000`, {
          headers: { 'Accept': 'application/json' }
        })
...
**數據處理指引：**
1. **嚴格禁止編造數據**：只能基於提供的真實餐廳銷售數據進行分析，絕對不可生成任何虛假數據或數字
2. **數據驗證要求**：如果用戶詢問特定數據（如"主食分類的每月金額"），必須先在提供的數據中查找對應信息
3. **缺失數據處理**：如果數據中沒有用戶詢問的特定項目，必須明確指出"根據現有數據，無法找到相關信息"，並建議類似項目
4. **數值準確性**：所有數值、金額、百分比必須來自真實數據，不可進行推測或估算
5. 提供具體的數值和趨勢分析，特別是月度變化
6. 對於商品問題，您現在使用統一的商品銷售API，包括：
...
10. 請提供表格格式的數據（如果相關）
11. **分類問題數據匹配要求**：您現在使用基於實際交易的分類分析，包括：
   - 完整商品銷售報表（salesData）：所有實際交易記錄（無限制抓取）
   - 完整商品主檔（productMaster）：所有1610個商品的分類信息（無限制抓取）
   - **必須**透過商品名稱匹配，將實際銷售數據與分類信息結合
   - **必須**提供基於真實交易的分類統計（如5主食分類的實際銷售金額）
   - **絕對禁止**編造任何分類數據，所有金額和佔比必須來自真實計算
   
12. **分類數據計算步驟**：
   - 步驟1：從salesData中找出所有交易記錄
   - 步驟2：從productMaster中找出每個商品的分類
   - 步驟3：透過商品名稱匹配，將交易金額歸類到對應分類
   - 步驟4：計算每個分類的總金額和佔比
   - 步驟5：只報告計算出的真實數據
13. 如果遇到數據獲取錯誤，請向用戶說明並建議聯繫系統管理員
14. **重要**：統一API確保所有分析都基於相同的數據源，消除了之前多數據源混用造成的數據不一致問題
...
**🚫 嚴格禁止行為：**
- 絕對不可編造或猜測任何數據、數字、金額或百分比
- 不可生成虛假的月度數據或趨勢
- 不可假設不存在的商品分類數據
- 如果數據不存在，必須誠實告知用戶
- 所有分析必須基於提供的真實數據進行

**⚠️ 重要提醒：用戶已反映系統生成了虛假的"主食分類每月金額"數據，請確保不再發生此類問題！**
`
